...
const Suggestion = () => {
    ...
    const fetchRandomDocument = async () => {
        ...
        const randomDocuments = await Promise.all(snapshot.docs.map(async (doc) => {
            const data = doc.data();
    ...
    const handleSendFriendRequest = (friendRequestSent, uid, name, username, idx) => {
        const currentUserUid = currentUser?.uid;
        const targetUserUid = uid;
        ...
        <div className="suggestion-main-container">
            ...
            <ul className="suggestion-user-container" ref={scrollRef}>
                {randomDocs.map((user, idx) => {
                    const { photoURL, username, name, uid, bio, friendRequestSent } = user;
                    return (
                        <li 
                        ...
                        >
                            ...
                            <div className="suggestion-user-data">
                                ...
                                <p className="suggestion-user-bio">
                                    {bio ? bio : "..."}
                                </p>
                                {uid !== currentUser?.uid && (
                                    <button
                                        ...
                                        onClick={(e) => {
                                            e.stopPropagation()
                                            handleSendFriendRequest(friendRequestSent, uid, name, username, idx)
                                        }}
                                    >
