...
const validate = {
  name : (value) => {
    if (!value)
      return {name : true, nameError : "Name field cannot be empty"};
    else {
  ...
  email : (value) => {
    return emailRegex.test(value) ? {email : false, emailError : false} : {
      email : true,
  ...
  password : (value) => {
    return passwordRegex.test(value) ? {password : false, passwordError : false} : {
      password : true,
      passwordError :
